package dev.gaphunter.envdiffcompanion.review
 ...
 * run a real number of successful comparisons -- never on install,
 * never on a timer. Every call to [recordHit] here already corresponds
 * to one completed comparison (a report was written -- both "same
 * keys" and "differences found" count, there's no no-op branch), so
 * this counts invocations directly instead of tracking a set of seen
 * keys like the detector-based plugins in this catalog do.
 *
...
object ReviewPrompt {
    ...
    /** How many successful comparisons before the prompt shows once. */
    private const val HITS_BEFORE_PROMPT = 5

    private const val KEY_HIT_COUNT = "dev.gaphunter.envdiffcompanion.review.hitCount"
    private const val KEY_ANSWERED = "dev.gaphunter.envdiffcompanion.review.answered"

    private const val NOTIFICATION_GROUP_ID = "Env Diff Companion"
    ...
    /**
     * Call this once per real, completed comparison. Safe to call on
     * any thread.
     */
